...
var txtEmp = document.getElementById("txtEmp");
...
function editRecord(e) {
    ...
    imgOk.addEventListener('click', function (e) {
        ...
        workerStore = indexTx.objectStore("WorkerStore");
        workerStorePut(updateKey, txtName.value, txtLastName.value, txtDate.value, txtQual.value, txtEmp.value);
    ...
    keys.forEach(function (key) {
        workerStore.get(key).onsuccess = function (e) {
            ...
            if (res == null) {
                indexTx = indexDB.transaction("WorkerStore", "readwrite");
                workerStore = indexTx.objectStore("WorkerStore");
                workerStorePut(key, workers[key].name, workers[key].lastName, workers[key].birthDay, workers[key].qualifications, workers[key].employers);
            }
        };
    });
    insertRows();
